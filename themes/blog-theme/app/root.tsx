...
export const loader: LoaderFunction = async ({ request }): Promise<SiteLoader> => {
  ...
  // blog-theme additions: pin_theme forces 'light' or 'dark' regardless of
  // the user's cookie or system preference.
  const pinTheme = (config as any)?.options?.pin_theme;
  const pinned = pinTheme === 'light' || pinTheme === 'dark' ? pinTheme : null;
  const data = {
    theme: pinned ?? themeSession.getTheme(),
    config,
...
export default function AppWithReload() {
  ...
  // blog-theme additions: when pin_theme is set, override the boot script that
  // auto-detects via prefers-color-scheme. We re-pin in a synchronous head
  // script so it runs before any other script and there's no flash.
  const pinTheme = (config as any)?.options?.pin_theme;
  const pinScript =
    pinTheme === 'light' || pinTheme === 'dark'
      ? `(() => {
          const c = document.documentElement.classList;
          c.remove('light'); c.remove('dark');
          c.add(${JSON.stringify(pinTheme)});
        })();`
      : null;
